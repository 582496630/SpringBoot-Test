...
public class ImgShowController {
		...
		if (!file.isEmpty()) {
			try {
				...
				String filePath = request.getSession().getServletContext()
				 .getRealPath("/") + filePathSql + file.getOriginalFilename();
				//把文件名放入list中
	...
	@ResponseBody
	public Ret deleteImg(@RequestParam(value="ids") String ids,HttpServletRequest request,
			Model model){
		try {
			String loginName = (String)request.getSession().getAttribute("loginname");
			if (StringUtils.isBlank(loginName)) {
				/*map.put(ERROR, "用户未登录，请先登录");
				return map;*/
