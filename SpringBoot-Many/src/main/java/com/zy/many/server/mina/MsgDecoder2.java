...
public class MsgDecoder2 implements ProtocolDecoder {
	private Charset charset; // 编码格式
	private String delimiter; // 文本分隔符
	private IoBuffer delimBuf; // 文本分割符匹配的变量 // 定义常量值，作为每个IoSession中保存解码任务的key值
	private static String CONTEXT = MsgDecoder2.class.getName() + ".context"; // 构造函数，必须指定Charset和文本分隔符

	public MsgDecoder2(Charset charset, String delimiter) {
